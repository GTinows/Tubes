...
import 'client/user_client.dart';  // Assuming this is where your API calls are made.
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  bool dataFetched = false;  // To prevent endless loop
  ...
  Future<void> _fetchUserData() async {
    if (dataFetched) return;  // Prevent calling the function multiple times
    ...
    try {
      final userData = await UserClient.fetchUser();  // Updated method to fetch user data
      print(userData);  // Log the data for debugging to check if data is being fetched

      setState(() {
        currentUser = userData;
        isLoading = false;
        dataFetched = true;  // Mark as data fetched to prevent further calls
      });
    } catch (e) {
      setState(() {
        isLoading = false;
        dataFetched = true;  // Mark as data fetched, even if the call fails
      });
  ...
  Widget build(BuildContext context) {
    ...
    // Safely calculate BMI using the null-aware operator (?.)
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 10),
              Text(
               currentUser?.username ?? "No Username",  // Safely access with null-aware operator
                style: TextStyle(
                  fontSize: 20,
              ...
              Text(
                currentUser?.email ?? "No Email",  // Safely access with null-aware operator
